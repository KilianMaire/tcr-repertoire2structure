...
_HARDENED_FOLD = _code(
    ...
    "\n",
    "SMOKE = True        # True: fold ONLY the first construct as a cheap live test. Verify\n",
    "                    # the log shows 'do not need to update msa result' (MSA server NOT\n",
    "                    # touched), then set False and re-run to fold the whole batch.\n",
    "TIMEOUT_S = 900     # kill a single fold after this many seconds. A healthy fold is\n",
    "                    # ~1-3 min (compile is cached after the first); this backstops a\n",
    "                    # slow/hung Protenix MSA server, which is still pinged per fold.\n",
    "\n",
    "def _run_fold(key):\n",
    ...
    "            os.remove(f)\n",
    "        rc, out = _run_fold(key)\n",
    "    if rc == 124:  # timed out, most likely a slow MSA server -> give it one more try\n",
    "        print('TIMEOUT -> retry once', key, flush=True)\n",
    "        rc, out = _run_fold(key)\n",
    "    if rc != 0:\n",
