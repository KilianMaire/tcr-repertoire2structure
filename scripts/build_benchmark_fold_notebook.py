...
_HARDENED_FOLD = _code(
    "# 3. fold each record with the provided MSA (unpairedMsaPath in the JSON). Self-heals a\n",
    "# truncated Protenix checkpoint: on the 'central directory'/PytorchStreamReader zip error\n",
    "# it deletes /root/checkpoint/*.pt and retries the key once so the weights re-download.\n",
    ...
    "    if r.returncode != 0 and ('central directory' in out or 'PytorchStreamReader' in out):\n",
    "        print('CHECKPOINT CORRUPT -> delete + re-download + retry', key, flush=True)\n",
    "        for f in glob.glob('/root/checkpoint/*.pt'):\n",
    "            os.remove(f)\n",
    "        r = _run_fold(key); out = r.stdout + r.stderr\n",
    "    if r.returncode != 0:\n",
    "        print('FAIL', key, out[-1500:]); continue\n",
    "    manifest[key] = sorted(glob.glob(f'out/{key}/**/*.cif', recursive=True))\n",
