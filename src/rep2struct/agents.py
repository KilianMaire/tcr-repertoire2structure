...
def _executor(name, tool):
    return AgentDefinition(
        ...
        prompt=(
            ...
            f"{{cid}}_cognate and {{cid}}_scramble: download the CIFs for BOTH constructs and "
            f"record ALL of them in one record_fold_result call, so QC can ensemble the cognate "
            f"samples and calibrate against the scramble null. The loop is resumable; skip jobs already recorded. If "
